...
def calculate_data_rate(time_differences, block_size):
    filtered_time_differences = [td for td in time_differences if td > 0 and td <= 3500000]  # Filter out outliers
    data_rates = [(block_size / (td / 1000)) for td in filtered_time_differences]  # B/s
    return data_rates
...
def plot_data_rate(data_rates, file_label, ax):
    ...
    ax.set_title(f'Data Rate Over Time (SPI 8 MHz)')
    ax.set_xlabel('Index')
...
def main():
    file_path = 'log_data.txt'  # Replace with your specific log file path
