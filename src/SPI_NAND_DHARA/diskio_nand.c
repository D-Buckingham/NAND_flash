...
int nand_disk_access_init(struct disk_info *disk) {
    ...
    int ret = spi_nand_flash_init_device(&nand_flash_config, &nand_flash_device_handle);
    return ret;
}
...
int nand_disk_access_read(struct disk_info *disk, uint8_t *data_buf, uint32_t start_sector, uint32_t num_sector) {
    ...
    int ret = spi_nand_flash_get_sector_size(nand_flash_device_handle, &sector_size);
    if (ret < 0) {
    ...
    for (uint32_t i = 0; i < num_sector; ++i) {
        ret = spi_nand_flash_read_sector(nand_flash_device_handle, data_buf + i * sector_size, start_sector + i);
        if (ret < 0) {
...
int nand_disk_access_write(struct disk_info *disk, const uint8_t *data_buf, uint32_t start_sector, uint32_t num_sector) {
    ...
    for (uint32_t i = 0; i < num_sector; ++i) {
        ret = spi_nand_flash_write_sector(nand_flash_device_handle, data_buf + i * sector_size, start_sector + i);
        if (ret < 0) {
...
int nand_disk_access_ioctl(struct disk_info *disk, uint8_t cmd, void *buff) {
    ...
    switch (cmd) {
        case DISK_IOCTL_GET_SECTOR_COUNT:
            ...
            ret = spi_nand_flash_get_capacity(nand_flash_device_handle, (uint32_t *)buff);
            if (ret < 0) {
        ...
        case DISK_IOCTL_GET_SECTOR_SIZE:
            ...
            ret = spi_nand_flash_get_sector_size(nand_flash_device_handle, (uint32_t *)buff);
            if (ret < 0) {
        ...
        case DISK_IOCTL_CTRL_SYNC:
            ...
            ret = spi_nand_flash_sync(nand_flash_device_handle);
            if (ret < 0) {
        ...
        case DISK_IOCTL_GET_ERASE_BLOCK_SZ:
            *(uint32_t *)buff = nand_flash_device_handle->block_size;
		    break;
