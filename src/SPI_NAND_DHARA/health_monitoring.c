...
static int wait_for_ready_nand( uint8_t *status_out)
{
    ...
    while (true) {
        uint8_t status;
        int err = spi_nand_read_register(device_handle->config.spi_dev, REG_STATUS, &status);
        if (err != 0) {
...
{
    int err;
    err = spi_nand_read_page(device_handle -> config.spi_dev, page); 
    if (err != 0) {
...
uint32_t read_bad_block_count(void) {
    ...
    for (int b = 0; b < device_handle->num_blocks; b++) {
        ...
        ret = spi_nand_read(device_handle->config.spi_dev, (uint8_t *)&bad_block_indicator, device_handle->page_size, 2);
        if (ret != 0) {
