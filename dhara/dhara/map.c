...
LOG_MODULE_REGISTER(mapdhara, CONFIG_LOG_DEFAULT_LEVEL);
...
not_found:
	...
	LOG_DBG("DHARA_E_NOT_FOUND, trace_path");
	return -1;
}
		   ...
		   uint8_t *data, dhara_error_t *err)
{
	...
	if (dhara_map_find(m, s, &p, &my_err) < 0) {
		if (my_err == DHARA_E_NOT_FOUND) {
			...
			LOG_DBG("DHARA_E_NOT_FOUND, dhara_map_read");
			return 0;
		}
			 ...
			 uint8_t *meta, dhara_error_t *err)
{
	...
	if (trace_path(m, dst, NULL, meta, &my_err) < 0) {
		if (my_err != DHARA_E_NOT_FOUND) {
			dhara_set_error(err, my_err);
			LOG_DBG("DHARA_E_NOT_FOUND, prepare_write");
			return -1;
		}
			  ...
			  dhara_sector_t dst, dhara_error_t *err)
{
	...
	if (dhara_map_find(m, src, &p, &my_err) < 0) {
		if (my_err == DHARA_E_NOT_FOUND){
			LOG_DBG("DHARA_E_NOT_FOUND, dhara_map_copy_sector");
		      ...
		      dhara_error_t *err)
{
	...
	if (trace_path(m, s, NULL, meta, &my_err) < 0) {
		if (my_err == DHARA_E_NOT_FOUND){
			LOG_DBG("DHARA_E_NOT_FOUND, try_delete");
			return 0;
		}
		dhara_set_error(err, my_err);
		return -1;
